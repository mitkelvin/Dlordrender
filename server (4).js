...
// ============================================================
// Devices are cached in memory for fast reads, but every write
// (register, sentToday increment, dead-token removal) is also
// persisted to Firestore. On startup, the cache is rebuilt from
// Firestore — so registered devices survive Render restarts,
// deploys, and free-tier spin-downs, and notifications keep
// working even after the app has been offline for days.
...
if (firebaseReady) {
  ...
  try {
    ...
    console.error(
      "[DLORD] Firestore initialization failed:",
      error.message
    );

    console.error(
      "[DLORD] Falling back to in-memory device storage only " +
...
async function loadDevicesFromFirestore() {
  ...
  try {
    ...
    console.log(
      `[DLORD] Loaded ${notificationDevices.size} ` +
      `device(s) from Firestore.`
    );
    ...
    console.error(
      "[DLORD] Failed to load devices from Firestore:",
      error.message
    );
...
async function saveDeviceToFirestore(token, deviceData) {
  ...
  try {
    ...
    console.error(
      "[DLORD] Failed to save device to Firestore:",
      error.message
    );
...
async function deleteDeviceFromFirestore(token) {
  ...
  try {
    ...
    console.error(
      "[DLORD] Failed to delete device from Firestore:",
      error.message
    );
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await saveDeviceToFirestore(
        token,
        notificationDevices.get(token)
      );
...
async function sendNotification(
  ...
  try {
    ...
    if (
      ...
      await deleteDeviceFromFirestore(
        device.token
      );
...
server.listen(
  ...
  async () => {
    ...
    // ========================================================
    // Restores registered devices from Firestore so they
    // survive restarts, redeploys, and free-tier spin-downs.

    await loadDevicesFromFirestore();
